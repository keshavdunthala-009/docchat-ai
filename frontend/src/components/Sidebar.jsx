...
export function Sidebar({ docs, activeDoc, questionCount, onUpload, onSelect }) {
    ...
    <div style={{background:'#111', borderRight:'1px solid #1e1e1e', display:'flex', flexDirection:'column', padding:'16px', gap:'14px', height:'100vh', overflow:'hidden'}}>
      ...
      <div style={{border:'1px dashed #2a2a2a', borderRadius:'10px', padding:'16px', textAlign:'center', background:'#161616'}}>
        ...
        <form onSubmit={handleUpload}>
          <input type='file' accept='.pdf,.txt' onChange={(e) => setFile(e.target.files[0])} style={{fontSize:'11px', marginBottom:'8px', width:'100%', color:'#888'}}/>
          <button type='submit' disabled={loading || !file} style={{width:'100%', padding:'7px', background: loading || !file ? '#1a1a1a' : '#6c47ff', color: loading || !file ? '#555' : '#fff', border:'1px solid #2a2a2a', borderRadius:'6px', cursor: loading || !file ? 'not-allowed' : 'pointer', fontSize:'11px'}}>
      ...
      {docs.length > 0 && (
        <>
          <div style={{fontSize:'10px', fontWeight:'600', color:'#444', letterSpacing:'0.8px', textTransform:'uppercase'}}>Documents</div>
          <div style={{display:'flex', flexDirection:'column', gap:'6px', overflowY:'auto'}}>
            {docs.map((doc, i) => (
              <div key={i} onClick={() => onSelect(doc)} style={{display:'flex', alignItems:'center', gap:'8px', padding:'9px 10px', borderRadius:'8px', background: activeDoc?.name === doc.name ? '#1a1535' : '#161616', border:'1px solid', borderColor: activeDoc?.name === doc.name ? '#6c47ff33' : '#1e1e1e', cursor:'pointer'}}>
                <span style={{fontSize:'14px', flexShrink:0}}>📄</span>
                <span style={{fontSize:'11px', color:'#ddd', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{doc.name}</span>
                {activeDoc?.name === doc.name && (
                  <span style={{fontSize:'9px', padding:'2px 7px', borderRadius:'99px', background:'#6c47ff22', color:'#9b7cff', border:'1px solid #6c47ff44', flexShrink:0}}>Active</span>
                )}
              </div>
            ))}
          </div>
