...
export function Sidebar({ docs, activeDoc, questionCount, onUpload, onSelect }) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const API_URL = 'https://docchat-ai-production-up.up.railway.app';
  ...
  return (
    <div style={{background:'#111', borderRight:'1px solid #1e1e1e', display:'flex', flexDirection:'column', padding:'16px', gap:'14px', height:'100vh', overflow:'hidden'}}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:'10px', paddingBottom:'14px', borderBottom:'1px solid #1e1e1e'}}>
        ...
        <div>
          <div style={{fontSize:'14px', fontWeight:'600', color:'#fff', letterSpacing:'0.3px'}}>DocChat AI</div>
      ...
      <div style={{border:'1px dashed #2a2a2a', borderRadius:'10px', padding:'16px', textAlign:'center', background:'#161616'}}>
        ...
        <form onSubmit={handleUpload}>
          ...
          <button 
            ...
            style={{width:'100%', padding:'7px', background: loading || !file ? '#1a1a1a' : '#6c47ff', color: loading || !file ? '#555' : '#fff', border:'1px solid #2a2a2a', borderRadius:'6px', cursor: loading || !file ? 'not-allowed' : 'pointer', fontSize:'11px', fontFamily:'Segoe UI, sans-serif'}}
          >
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'6px', overflowY:'auto'}}>
            {docs.map((doc, i) => (
              <div 
                ...
                style={{display:'flex', alignItems:'center', gap:'8px', padding:'9px 10px', borderRadius:'8px', background: activeDoc?.name === doc.name ? '#1a1535' : '#161616', border:'1px solid', borderColor: activeDoc?.name === doc.name ? '#6c47ff33' : '#1e1e1e', cursor:'pointer'}}
              >
